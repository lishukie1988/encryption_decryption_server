...
void sendMessage(int socket, char *message) {
  ...
  while(1) {
    ...
    if (charsWritten < 0){
      fprintf(stderr,"ENC_SERVER: ERROR writing to socket\n"); 
...
void put_key(char *buffer, char *key) {
  ...
  int length = end_of_key - end_of_plaintext - 1; 
  // copy key to the target buffer
  strncpy(key, end_of_plaintext + 1, length);
...
*/
char *encryptPlaintext(char *plaintext, char *key) {

  // initialize buffer for the ciphertext
  char *cypher_text = malloc(strlen(plaintext) + 1);
  memset(cypher_text, '\0', strlen(plaintext) + 1);
  ...
  for (int x = 0; x < strlen(plaintext); x++) {
    ...
    int plaintext_num = asciiToNum(plaintext + x);
    int key_num = asciiToNum(key + x);
    // derive the resulted encrypted value
    int outcome_num = plaintext_num + key_num;
    // if value > 26
    if (outcome_num > 26) {
      // subtract it by 27, the number of available characters
      outcome_num -= 27;
    }
    // put completed character into the buffer for the ciphertext
    cypher_text[x] = *numToAscii(outcome_num);
  }
